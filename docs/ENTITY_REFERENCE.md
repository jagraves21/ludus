...
A [GameEngine](#gameengine) is a framework service responsible for coordinating the execution of a game session. A [GameEngine](#gameengine) manages the execution lifecycle by requesting [Moves](#moves), submitting [Moves](#moves) to the [RulesEngine](#rulesengine) for evaluation, processing [MoveResolutions](#moveresolutions), replacing the current [GameState](#gamestate) with resulting immutable [GameStates](#gamestates), and updating [GameHistory](#gamehistory). A [GameEngine](#gameengine) does not contain game-specific rules, interpret the meaning of [Moves](#moves), or directly modify existing [GameStates](#gamestates). Instead, it coordinates the interaction between execution services and domain concepts while preserving the framework guarantees of immutable state transitions and deterministic execution.

##### Responsibilities

- coordinate game execution
- provide execution context for a running [GameInstance](#gameinstance)
- request [Moves](#moves) from [MoveProviders](#moveprovider)
- submit [Moves](#moves) to the [RulesEngine](#rulesengine)
- process [MoveResolutions](#moveresolutions)
- replace the current [GameState](#gamestate) with resulting immutable [GameStates](#gamestates)
- update [GameHistory](#gamehistory)
...
A [MoveProvider](#moveprovider) is a framework service responsible for providing [Moves](#moves) to a [GameEngine](#gameengine) during game execution. A [MoveProvider](#moveprovider) represents the source of a decision within a game session and may provide [Moves](#moves) generated by a human player, artificial intelligence agent, network client, replay system, or automated process. A [MoveProvider](#moveprovider) does not evaluate whether a [Move](#move) is valid, apply game rules, modify [GameStates](#gamestates), or control game execution. Instead, it produces [Move](#move) requests that are evaluated by the [RulesEngine](#rulesengine) through the [GameEngine](#gameengine).

##### Responsibilities

- provide [Moves](#moves) during game execution
